...
Autorzy {ID autora, Imiê, Nazwisko}
Ksi¹¿ki (ID ksi¹¿ki, Tytu³}
AutorKsia¿ka {ID autora, ID ksi¹¿ki}

Uczniowie {Uczeñ ID, Imiê, Nazwisko, Adres ID, Uwagi}
Oceny {Ocena ID, Ocena}
UczniowieOceny {Uczeñ ID, Ocena ID, Data wystawienia oceny}
...
SELECT K.Imiê, K.Nazwisko
FROM dbo.Klienci AS K JOIN dbo.Zamówienia AS Z
ON K.ID = Z.[ID klienta]
WHERE Z.[ID klienta] IS NULL;
SELECT K.Imiê, K.Nazwisko
FROM dbo.Klienci AS K LEFT JOIN dbo.Zamówienia AS Z
ON K.ID = Z.[ID klienta]
WHERE Z.[ID klienta] IS NULL;
GO
